...
Every finding, by human, machine, or AI, is anchored to those coordinates and filed in the record. It lives on the document, points at the text that proves it, and holds its position as the corpus grows and changes around it.

## Three kinds of authors. One record.

**Deterministic passes flag whatever you need flagged.** Every citation, every mention of a name, every dollar figure, every image — anything that can be identified mechanically is filed across the whole corpus at once, each occurrence anchored to its exact location. This creates the scaffolding the other authors build on.
...
Exeges models what sources do to each other, so a change arrives with its consequences attached. Search is a traversal of typed relationships, each one clickable to the text that proves it. Ask for an exact set instead of a ranking and you get one: *"every provision that adds a requirement and rescinds an exemption"* returns a proven set, selected by compound membership, ordered by document order. Plain-language questions land in the same place: sections open in the corpus with prior findings already visible on the passage the answer landed on.
...
Analyst and agent findings accumulate on the corpus, so provisional machine output stays distinguishable from validated conclusions and contested ground stays visible rather than silently overwritten. When sources change, the findings anchored to them are re-tested instead of silently going stale — the record keeps up with the world it describes. New hires and new agent sessions inherit everything the team has established instead of re-deriving it.
